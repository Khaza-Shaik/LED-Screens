    ...
    <div className="min-h-screen app-bg text-zinc-900 pt-32 px-6">
      <div className="max-w-7xl mx-auto">
        <header className="mb-12">
          ...
          <p className="text-zinc-600 text-lg">Flexible plans for campaigns of any size.</p>
        </header>
          ...
          {plans.map((plan) => (
            <div key={plan.name} className="glass-card rounded-3xl p-8 border border-white/10">
              <div className="flex items-center justify-between mb-6">
                ...
                {plan.name === 'Growth' && <Sparkles className="text-primary" size={18} />}
              </div>

              <p className="text-4xl font-black mb-8">
                {plan.price}
                <span className="text-lg font-medium text-zinc-500">{plan.duration}</span>
              </p>

              <ul className="space-y-4 mb-8">
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-center gap-3 text-zinc-900/80">
              ...
              <button className="w-full py-3 rounded-2xl bg-primary text-black font-bold hover:scale-[1.02] transition-transform">
                Choose {plan.name}
              </button>
            </div>
          ))}
